...
DB_SQL_DATA_QUERY="select customer_name, city from customers ORDER BY customer_name;"
...
function insert_array_data() {
    ${DB_Connect} <<EOF >insert_array_data_$current_date.log
    set lines 12345 pages 12345;
    col username for a30;
    col open_mode for a30;
    INSERT ALL
        INTO ABCDEF.shipers VALUES ($1, '$2', '$3')
        INTO ABCDEF.orders VALUES ($1, '$2', '$3')
    SELECT * FROM dual;
...
if [ ${#listCity[*]} > 0 ]; then
    ...
    do
       id=`expr $i + 1`
       echo $id
       insert_array_data $id ${lstCusName[$i]} ${listCity[$i]}
    done
fi
